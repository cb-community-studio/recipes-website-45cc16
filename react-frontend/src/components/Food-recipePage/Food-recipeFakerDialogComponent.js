...
import food-recipeFakerFactory from "./food-recipeFakerFactory";
...
const Food-recipeFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = food-recipeFakerFactory(fakerCount);
        let results = [];
        setLoading(true);
        try {
            for (let i = 0; i < fakeData.length; i++) {
                const _data = fakeData[i];
                const res = await client.service("food-recipe").create(_data);
                // const res = _data;
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(Food-recipeFakerDialogComponent);
